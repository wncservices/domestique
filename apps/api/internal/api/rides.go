...
type rideDTO struct {
	...
	// SeriesID is set when this ride was generated by a recurring series —
...
func (s *Server) handleCreateRide(w http.ResponseWriter, r *http.Request) {
	...
	route, ok := s.resolveRideRoute(w, r, id, body.Slug, identity, crews)
	if !ok {
		return
	}
...
// sharing it with the crew first if it isn't already and the caller has
// the authority to do so — the rule handleCreateRide's own doc comment
// describes. Factored out so handleCreateRideSeries can call it exactly
// once per series rather than once per generated occurrence: sharing a
// route with a crew is a property of the route, not of any one ride
// scheduled against it.
func (s *Server) resolveRideRoute(w http.ResponseWriter, r *http.Request, crewID, slug string, identity auth.Identity, crews crew.Snapshot) (model.Route, bool) {
	routes, _, err := s.Source.List(r.Context())
	if err != nil {
		s.fail(w, err)
	...
	shared := false
	if route.Targets != nil && crews.ApprovedRiders.Has(crewID, route.Owner) {
	...
	newTargets := append(append([]string{}, existing...), crewID)
...
// handleCreateRideSeries schedules a recurring series of rides — the same
// canSchedule authority and route-sharing rule as handleCreateRide (see
// its own and resolveRideRoute's doc comments), applied once for the whole
// series rather than once per occurrence.
func (s *Server) handleCreateRideSeries(w http.ResponseWriter, r *http.Request) {
	if !s.require(w, r, auth.PermManageCrews) {
		return
	}
	if !s.crewAvailable(w) || !s.scheduleAvailable(w) {
	...
	_, rides, err := s.Schedule.CreateSeries(r.Context(), id, route.Slug,
		body.IntervalWeeks, body.Date, body.Until, body.Time, identity.User)
	if err != nil {
		writeJSON(w, http.StatusBadRequest, map[string]string{"error": err.Error()})
		return
	}

	names := map[string]string{route.Slug: route.Name}
	out := make([]rideDTO, 0, len(rides))
	for _, ride := range rides {
		out = append(out, rideDTOFor(ride, names))
	}
...
// handleDeleteRideSeries cancels every not-yet-happened ride in a series —
// the same authority as handleDeleteRide (the crew's owner/admin, or the
...
func (s *Server) handleDeleteRideSeries(w http.ResponseWriter, r *http.Request) {
	...
	id, seriesID := r.PathValue("id"), r.PathValue("seriesId")
	c, err := s.Crew.Get(r.Context(), id)
	if err != nil {
		s.failCrewLookup(w, err)
		return
	...
	// A series has no CreatedBy of its own to check the way one ride
	// does — created_by is identical across every ride it generated — so
	// this reads it off any one ride still carrying that series id.
	rides, err := s.Schedule.ListForCrew(r.Context(), id)
	if err != nil {
		s.fail(w, err)
		return
	...
	if !found {
		writeJSON(w, http.StatusNotFound, map[string]string{"error": "no such ride series"})
		return
	}
	if !authority.mine && (!authority.approved || !strings.EqualFold(createdBy, identity.User)) {
		s.forbidCrew(w, r)
		return
	}
	...
	n, err := s.Schedule.DeleteSeries(r.Context(), seriesID, from)
	if err != nil {
		s.fail(w, err)
		return
	}

	s.logger().Info("crew ride series cancelled", "crew", id, "series", seriesID, "removed", n, "by", identity.User)
